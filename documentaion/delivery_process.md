...
- **Student-facing slides stay in-world** — same rule as the intranet. No course/assessment language *on the slide*: no "AT1 / Appendix 2 / Business Case §x", and nothing that tips **which system is the assessed one** (students practise on the Accounting System; the real engagement is handed out later). Put the UoC / AT / appendix mapping in the **speaker/trainer notes** only. **Exception (Tim, 2026-06-03): AWS source references may remain visible to students** — the deck/slide provenance in a teach slide's kicker (e.g. "AWS · ACF M06 S10"), the image-placeholder labels, and the demo `source=` cue. They're attribution to the reused AWS material (and a build/trainer aid), not course/assessment meta — analogous to the sanctioned UoC footer on migrated intranet docs.
