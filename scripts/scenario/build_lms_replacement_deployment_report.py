...
def build(path):
    ...
    add_body_paragraph(doc, "AccentLoitte deployed the DOODLE LMS on-premises at the YAT Cremorne campus, replacing the "
    ...
    add_body_paragraph(doc, "Acceptance testing confirmed DOODLE meets the ≥ 99% availability target, matches or "
    ...
    add_body_paragraph(doc, "This report covers Phases 2–3 of the LMS Replacement engagement — deployment and data "
    ...
    add_body_paragraph(doc, "In scope: provisioning the on-premises server, installing and configuring DOODLE, "
    ...
    add_body_paragraph(doc, "Cutover was a change to the live LMS, performed during the December–January teaching break "
    ...
    add_body_paragraph(doc, "DOODLE was integrated with Active Directory via LDAP bind for single sign-on; the existing "
    ...
    h3("4.2 Network")
    add_body_paragraph(doc, "The server was placed in the staff network zone, with student-zone access to the LMS "
    ...
    add_body_paragraph(doc, "A new mid-range enterprise server (dual PSU, RAID-protected disks) was provisioned with "
    ...
    add_body_paragraph(doc, "MySQL was installed on the server and the DOODLE schema created. The GrayBoard student "
                 ...
                 "fidelity (see §6.3).")
    h3("4.6 Storage")
    add_body_paragraph(doc, "RAID-protected local disk holds the OS, the DOODLE application and the MySQL database; the "
                 "campus NAS (RAID-5) holds course attachments, student submissions and backup copies.")
    h3("4.7 Security")
    add_body_paragraph(doc, "Access is controlled by Active Directory permissions and the two-zone separation, with "
    ...
    h3("4.8 Monitoring")
    add_body_paragraph(doc, "DOODLE was brought under the existing YAT system-management and monitoring server, with "
    ...
    add_body_paragraph(doc, "The application connected to MySQL successfully. Post-migration record counts reconciled "
    ...
    add_body_paragraph(doc, "DOODLE was reachable and login succeeded via AD. Load testing at ~3× typical concurrent "
    ...
    add_body_paragraph(doc, "Availability was tracked through the stabilisation period via the system-management "
    ...
    h3("7.1 Access")
    add_body_paragraph(doc, "Administrative access was handed to YAT ICT; AD-based access for staff and students "
    ...
    add_body_paragraph(doc, "The build evidence — server configuration, network placement, the DOODLE installation, and "
    ...
    add_body_paragraph(doc, "Server, database, storage and backup configuration records are filed with the as-built "
                 "documentation.")
    h1("Appendix C — Test and migration evidence")
    add_body_paragraph(doc, "The data-migration reconciliation report, the performance test results, and the WCAG 2.1 "
