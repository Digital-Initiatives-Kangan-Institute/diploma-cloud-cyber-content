...
The worked model answers live in the YAT-branded exemplars (build_at1_*_exemplar); this
document carries the task instructions and the marking guide with bidirectional UoC
traceability.
...
OVERVIEW = (
    "Students are assessed on the cloud design and disaster-recovery planning for the YAT LMS global "
    "expansion. Working as an MP Tech Solutions (MTS) consultant, the student produces a Solution Design "
    "(Part A) and a Disaster Recovery Plan (Part B) for the expanded LMS, then presents both to YAT ICT "
    "management for approval (Part C). This is the design-and-planning phase of the engagement; the approved "
    "design and plan are implemented in AT2."
)

TASKS = [
    "In this project the student takes on the role of an MTS consultant engaged by YAT College to design "
    "the changes that prepare the LMS for its India-campus expansion and to plan its disaster recovery. "
    "The project has three parts:",
    "• Part A — Solution Design: a written design covering the web-scale architecture (serving a "
    "global user base) and an audit-log microservice. Produced in the YAT Solution Design template.",
    "• Part B — Disaster Recovery Plan: a written DR plan for the designed system — risk and "
...
RESOURCES = [
    ...
    "• The YAT scenario site / intranet — the LMS Global Expansion project (engagement brief, "
    "requirements, ICT manager consultation notes, data residency & sovereignty requirements), the ICT "
    "documents (cloud application specification, baseline solution design), and the deprecated on-prem DR plan.",
    "• The YAT Solution Design template and YAT Disaster Recovery Plan template (intranet Templates section).",
    "• AWS Academy Cloud Foundations [104469] + AWS Academy Cloud Architecting [172221] — authorised "
    "lab environments (referenced for the design; the build itself is AT2).",
...
CONDITIONS = [
    ...
    "C3 — AWS Academy lab access is available for the student to reference cloud services in the design.",
...
PART_A = [
    "A1 — Purpose and scope: states the two design changes (scale for a global user base; add the "
    "audit-log microservice) and what is in and out of scope.",
    ...
    "A3 — Review of the existing architecture: reviews the current cloud baseline against the requirements "
    ...
    "A5 — Web-scale design — global delivery: designs the changes needed to serve a global user base.",
    "A6 — Web-scale design — availability and security: confirms availability and security are "
    "maintained through the changes, and reviews the design against the requirements.",
    ...
    "to the underlying web-scale concepts.",
...
PART_B = [
    ...
    "(risk environment, RTO/RPO, DR techniques, standards).",
]
...
PART_C = [
    ...
    "choices (the verbal knowledge-evidence location).",
    "C6 — Oral communication: uses listening and questioning techniques and industry language appropriate "
    "to the audience.",
]

# Benchmark: per-criterion UoC evidenced (bidirectional traceability).
BENCHMARK = [
    ("Part A — Solution Design (ICTCLD503 design)", [
        ...
        ("A7", "[ICTCLD503 KE 3] functions/benefits/differences of web-scale components; [KE 6] web-scaling "
               "principles and technologies; [PE 5] apply web-scaling principles."),
        ...
        ("A10", "[ICTCLD503 PC 2.3] design microservice architecture using cloud services; [KE 4] web-scale "
                "infrastructure (cohesive/coupled, database/storage, API/messaging/queuing)."),
        ("A11", "[ICTCLD503 PC 1.7] document and justify architecture changes; [PC 2.4] document and justify "
                "architecture design; [FS Writing]."),
        ("A12", "[ICTCLD503 KE 3, 4, 6] evidenced as written contextual responses against the student's design."),
    ...
    ("Part B — DR Plan (ICTCLD501)", [
        ...
        ("B2", "[ICTCLD501 PC 1.2] determine existing organisational recovery plans; [PC 1.3] identify vendor "
               "DR plan and SLAs."),
        ("B3", "[ICTCLD501 PC 2.1] determine RTO and RPO according to business needs; [KE 5] RTO/RPO standards "
               "and techniques."),
        ("B4", "[ICTCLD501 PC 2.3] estimate amount and security level of data managed."),
        ("B5", "[ICTCLD501 PC 2.4] evaluate severity of impact and disruption; [KE 1] risk environments; "
               "[KE 2] data-analysis methodologies; [PE 2] determine likelihood and impact of risk events."),
        ...
        ("B8", "[ICTCLD501 PC 3.1] develop a range of DR solutions; [KE 3] DR techniques for cloud environments."),
        ...
        ("B11", "[ICTCLD501 PC 3.4] identify other DR solution components; [KE 4] ISO 27001/27002/27031 standards."),
        ("B12", "[ICTCLD501 PC 4.1] align DR risk potential to business requirements; [PC 4.2] outline steps, "
                "timelines, key features, service providers; [KE 6] techniques/methods to monitor and create alerts."),
        ...
        ("B15", "[ICTCLD501 KE 1, 2, 3, 4, 5] evidenced as written contextual responses against the student's plan."),
    ]),
    ("Part C — Presentation (ICTCLD501 element 5)", [
        ...
        ("C5", "[ICTCLD501 KE 1–6] / [ICTCLD503 KE 3, 4, 6] verbal knowledge-evidence on the student's own "
...
STUDENT_INTRO = [
    ...
    ("YAT College is a Registered Training Organisation (RTO) based at 175 Cremorne Street, Cremorne VIC. "
     "Following a new partnership with a training institution in India, YAT is expanding its Learning "
     "Management System (LMS) to serve students in both countries.", "Assessor text"),
    ("You are an MP Tech Solutions (MTS) consultant reporting to Pat Lin (MTS Senior Consultant). Pat liaises "
     "with Sam Walker, the YAT ICT Manager. In AT1 you design the changes the expansion needs and plan the "
     "LMS's disaster recovery, then present both to YAT ICT management for approval. The approved design and "
     "plan are implemented in AT2.", "Assessor text"),
]
...
STUDENT_PART_A = [
    ...
    ("Using the YAT Solution Design template (download from the intranet's Templates section), produce a "
     "solution design for the LMS global expansion covering two strands:", "Assessor text"),
    ("• Web-scale design — the architecture changes that let the LMS serve a global user base while "
     "scaling network, compute and storage and maintaining availability and security.", "Assessor text"),
    ...
    ("Treat the data residency requirements as an input that shapes your design — do not write a separate "
     "compliance plan. Complete the Knowledge Evidence appendix with contextual short answers about your own "
     "design choices.", "Assessor text"),
    ("Resources for Part A — on the YAT intranet: the LMS Global Expansion project (engagement brief, "
     "requirements, ICT manager consultation notes, data residency & sovereignty requirements), the cloud "
     "application specification and baseline solution design, and the YAT Solution Design template.", "Assessor text"),
    ("Part A is submitted to the LMS as the populated Solution Design (.docx) with the Knowledge Evidence "
     "appendix completed.", "Assessor text"),
]
...
STUDENT_PART_B = [
    ...
    ("Using the YAT Disaster Recovery Plan template (download from the intranet's Templates section), produce a "
     "disaster recovery plan for the LMS designed in Part A. Disaster recovery answers ‘what do we do if "
     "the system goes down’ — keep it focused on recovery, not on the design or the regulatory "
    ...
    ("Your plan must cover: the plan requirements and current recovery position; an impact analysis with your "
     ...
     "service providers. Complete the Knowledge Evidence appendix with contextual short answers.", "Assessor text"),
    ("A deprecated on-premises DR plan is on the intranet — it was superseded by the cloud migration and "
     "needs replacing. Use it for context only; your plan is a new cloud DR plan.", "Assessor text"),
    ("Part B is submitted to the LMS as the populated Disaster Recovery Plan (.docx) with the Knowledge "
     "Evidence appendix completed.", "Assessor text"),
]
