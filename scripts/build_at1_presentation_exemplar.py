...
communication, and it is the verbal location for the AT1 contextual knowledge-evidence Q&A.
...
input the design answers — DR stays onshore and separate from compliance.
...
def build(path):
    ...
    b.add_title_slide(
        prs, "Design & Disaster Recovery Plan", "YAT LMS Global Expansion",
    ...
    b.add_content_slide(prs, "Agenda", [
        ...
        ("Web-scale design — serving a global cohort", 0),
    ...
    b.add_content_slide(prs, "Why now — the India expansion", [
        ("YAT's GIFT City partnership takes the LMS global — a partner institution now depends on it.", 0),
        ("That means the LMS must now: serve a global user base, survive the loss of a whole region, and be provisioned as code.", 0),
        ("Today's design is single-region high availability — right for one Australian cohort, not for this.", 0),
    ...
    b.add_content_slide(prs, "What we're designing", [
        ("Solution Design (Part A) — scale the web application for a global cohort, plus an audit-log microservice.", 0),
        ("Disaster Recovery Plan (Part B) — recover the LMS if the primary region is lost.", 0),
        ("Recovery objectives: RTO ≤ 4 hours, RPO ≤ 1 hour.", 1),
        ("This is the design. Approval today is the gate to building it in Phase 2.", 0),
    ])

    b.add_content_slide(prs, "Web-scale design — serving a global cohort", [
        ("CloudFront edge delivery + Route 53 latency routing — India users are served from the nearest edge, not Sydney.", 0),
        ("ElastiCache caches sessions and hot reads, absorbing a larger, spikier load off the database.", 0),
        ("The existing multi-AZ load balancer, Auto Scaling and database are retained; scaling policies retuned.", 0),
        ("Availability and security are maintained — WAF at the edge, the origin locked to CloudFront, TLS throughout.", 0),
    ])
    ...
    b.add_content_slide(prs, "The audit-log microservice", [
        ...
        ("Loosely coupled — the LMS only calls a webhook; the service can change independently behind it.", 0),
        ("The same generic engine serves the Accounting practice project — only the payload differs.", 0),
    ])
    ...
    b.add_content_slide(prs, "Disaster recovery — risks and objectives", [
        ...
        ("RTO ≤ 4 hours — the LMS is back in service within four hours of an incident being declared.", 0),
    ...
    b.add_content_slide(prs, "Disaster recovery — the recovery plan", [
        ...
        ("The runbook: detect → declare → rebuild from code → restore the database → verify → cut DNS over → confirm.", 0),
    ...
    b.add_content_slide(prs, "What stays onshore", [
        ("Recovery is into a second Australian region — student data never leaves the country, even during a disaster.", 0),
    ...
    b.add_statement_slide(
        ...
        "Design approved → Phase 2 implementation (the Deployment Report).")

    b.add_closing_slide(prs, "Questions", "MTS Consulting  ·  YAT LMS Global Expansion")
