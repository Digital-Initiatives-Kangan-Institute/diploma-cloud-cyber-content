...
Usage:  python scripts/build_at3_ha_deployment_exemplar.py [output.docx]
...
def build(path):
    ...
    ex.para(doc, "This report documents the high-availability hardening of the YAT LMS, applied to the "
    ...
    ex.para(doc, "Cross-Region active-active DR and application-layer clustering remain out of scope (noted "
                 "in §7.3). This report, with the companion HA Design (Part A), closes out MTS's role on the "
                 "engagement.")

    h1("2. Engagement Context")
    ex.para(doc, "This phase implements the YAT LMS Cloud Architecture — HA Design (Part A of this AT3 "
    ...
    ex.para(doc, "In scope: Multi-AZ database, cross-AZ compute and load-balancer resilience, HA-tuned "
    ...
    ex.para(doc, "The hardening was performed in a single simulated Saturday late-night maintenance window of "
    ...
    ex.uoc(doc, "[ICTCLD502 PC 4.1] (implement the design) · [ICTCLD502 PE 1, PE 2, PE 4]")
    h3("4.1 Identity and access management (changes from baseline)")
    ex.para(doc, "Minimal change: I added an AWS Backup service role and a cross-Region copy role for the "
                 "backup plan. No change to the human IAM groups or MFA enforcement from the AT2 baseline.")
    a2.ev(doc, "SCREENSHOT", "should show the AWS Backup service role / cross-Region copy role added; "
                            "region ap-southeast-2.")
    h3("4.2 Network topology (cross-AZ subnets)")
    ex.para(doc, "I added three subnets in ap-southeast-2b — public-web-b, private-app-b, private-data-b — "
    ...
    a2.ev(doc, "SCREENSHOT", "should show all six subnets across ap-southeast-2a and 2b with CIDRs/AZ, and "
                            "the second NAT Gateway.")
    h3("4.3 Compute (cross-AZ Auto Scaling)")
    ex.para(doc, "I extended the ASG across both AZ subnets and raised capacity to min 2 / desired 2 / max 4, "
                 "so the loss of one AZ still leaves a healthy instance serving while the ASG rebuilds "
                 "capacity in the survivor.")
    a2.ev(doc, "SCREENSHOT", "should show the ASG spanning both AZs with min/desired/max ≥ 2 and instances "
                            "healthy in each AZ.")
    h3("4.4 Application Load Balancer (cross-AZ targets)")
    ex.para(doc, "I extended the ALB's subnet group to both AZs and confirmed the target group reports "
                 "healthy targets in each AZ, so the ALB can route around an AZ-level failure.")
    a2.ev(doc, "SCREENSHOT", "should show the ALB target group with healthy targets in both ap-southeast-2a "
                            "and 2b.")
    h3("4.5 Database (Multi-AZ)")
    ex.para(doc, "I enabled Multi-AZ on the RDS instance; AWS provisioned a synchronous standby in "
    ...
    a2.ev(doc, "SCREENSHOT", "should show the RDS instance with Multi-AZ = Yes and the standby AZ; and the "
                            "RDS event log entry for the Multi-AZ conversion.")
    h3("4.6 Storage (cross-Region backup)")
    ex.para(doc, "I configured an AWS Backup plan with a cross-Region copy of the RDS and EBS snapshots to "
    ...
    a2.ev(doc, "SCREENSHOT", "should show the AWS Backup plan with the cross-Region copy rule to "
                            "ap-southeast-4.")
    h3("4.7 Security (HA-related adjustments)")
    ex.para(doc, "No security-group changes were required — cross-AZ traffic already flows within the VPC "
                 "under the existing tiered rules, and the new subnets inherited the tier-appropriate groups.")
    h3("4.8 Monitoring (HA-tuned)")
    ex.para(doc, "I added HA-tuned CloudWatch alarms beyond the AT2 baseline: a per-AZ healthy-host count, "
    ...
    a2.ev(doc, "SCREENSHOT", "should show the HA-tuned alarms list and the service-level availability "
                            "dashboard.")
    h3("4.9 Cross-Region backup / replication")
    ex.para(doc, "Completed — see §4.6 (AWS Backup cross-Region copy to ap-southeast-4).")
    ...
    ex.uoc(doc, "[ICTCLD502 PC 4.2–4.6, 5.1] · [ICTCLD502 PE 3, PE 5]")
    h3("6.1 Connectivity tests (regression)")
    ex.para(doc, "I re-ran the AT2 connectivity, autoscaling, database and smoke tests after the HA changes; "
    ...
    ex.para(doc, "Autoscaling, database connectivity, and the end-to-end smoke test from the foundation phase "
                 "were re-verified and passed; detailed below is the HA-specific simulation work that is the "
                 "core of this phase.")
    h3("6.5 Failure simulation")
    ex.etable(doc, ["#", "Simulation", "Method", "Observed outcome", "Reachable?", "Evidence"],
    ...
    ex.etable(doc, ["#", "Simulation", "Method", "Observed outcome", "Availability impact"],
    ...
    ex.para(doc, "I measured availability two ways during the window: a CloudWatch service-level dashboard, "
    ...
    a2.ev(doc, "TEST EVIDENCE", "should show the CloudWatch availability graph over the window and the "
                               "curl-loop summary (failed/total requests) used to compute the dips.")
    h3("6.8 Simulation findings vs the design")
    ex.etable(doc, ["Simulation", "Expected (HA Design)", "Actual", "Divergence?"],
    ...
    ex.para(doc, "The RDS failover ran ~15 s longer than the design predicted. Investigation (correlating the "
    ...
    ex.uoc(doc, "[ICTCLD401 PC 4.3] · [ICTCLD502 PC 5.2, 5.3]")
    h3("7.1 Access")
    ...
    ex.uoc(doc, "[ICTCLD502 KE 4, 5, 6, 7, 8, 9]")
    h3("Q1 — HA concepts in your design")
    ex.para(doc, "Single point of failure: the AT2 baseline's single RDS instance and single AZ were SPOFs; "
    ...
    ex.para(doc, "To avoid causing a single point of failure during testing, before terminating all "
    ...
    ex.para(doc, "I used CloudWatch metrics and alarms, the ALB and RDS event logs, and a curl loop against "
    ...
    ex.para(doc, "S3: fault tolerance is built in by AWS — objects are stored redundantly across AZs with no "
    ...
    ex.para(doc, "The combination delivers more than either alone: the ALB stops routing to an unhealthy "
    ...
    ex.para(doc, "I set up a per-AZ healthy-host-count alarm that didn't exist in the AT2 baseline. It "
    ...
    ex.uoc(doc, "[ICTCLD502 FS Self-management] · [ICTCLD401 FS Learning, Planning and organising]")
    h3("R1 — Decisions in hindsight")
    ex.para(doc, "Applying Multi-AZ immediately inside the maintenance window was the right call — it cost a "
    ...
    ex.para(doc, "When the F2 failover ran ~15 s longer than the design predicted, with the application "
