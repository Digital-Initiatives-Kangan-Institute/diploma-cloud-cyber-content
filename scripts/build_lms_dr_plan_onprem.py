...
Usage:  python scripts/build_lms_dr_plan_onprem.py [output.docx] [--deprecated]
...
def deprecated_banner(doc):
    ...
    bc.shade_cell(cell, "F6E5DF")  # pale terracotta
    bc.set_cell_borders(cell, bc.TERRACOTTA, sz=12)
    p = cell.paragraphs[0]
    h = p.add_run("DEPRECATED — REPLACEMENT REQUIRED")
    h.bold = True; h.font.size = Pt(12); h.font.color.rgb = RGBColor.from_string(bc.TERRACOTTA)
    ...
    b.font.size = Pt(10); b.font.color.rgb = RGBColor.from_string(bc.CHARCOAL)
    doc.add_paragraph()
...
def build(path, deprecated=False):
    ...
    sub.font.size = Pt(15); sub.bold = True; sub.font.color.rgb = RGBColor.from_string(bc.TEAL)
    doc.add_paragraph()
    if deprecated:
    ...
    ex.para(doc, "This plan sets out the disaster recovery arrangements for the YAT on-premises Learning "
    ...
    ex.para(doc, "Current recovery performance — a worst-case Recovery Time Objective of 7–11 hours and a "
    ...
    ex.para(doc, "The YAT LMS (the DOODLE application on Windows Server 2016 with a MySQL database) runs "
    ...
    ex.para(doc, "In scope: the LMS server, its MySQL database, the LMS application, and the LMS attachments "
    ...
    ex.para(doc, "A complete, full backup of the LMS environment runs nightly (22:00 – 04:00 local time), "
    ...
    ex.para(doc, "Recovery depends on the offsite tape-storage provider (tape retrieval to the Cremorne "
    ...
    ex.para(doc, "The ICT Strategic Plan sets the targets at RTO ≤ 4 hours and RPO ≤ 1 hour. The current "
                 "on-premises process achieves a worst-case RTO of 7–11 hours and an RPO of up to 24 hours — "
                 "both materially short of target.")
    ex.etable(doc, ["Objective", "Current achievement", "Target"],
              ...
              widths=[4.0, 7.5, 4.0])
    h3("4.2 Data managed")
    ex.para(doc, "The protected data comprises the MySQL DOODLE database (schema and content) and the LMS "
    ...
    ex.etable(doc, ["#", "Risk event", "Likelihood", "Impact", "Severity", "What it drives"],
    ...
    h3("4.4 Exclusions")
    ex.para(doc, "This plan excludes unrelated campus systems (covered by their own arrangements) and routine "
    ...
    ex.para(doc, "The current strategy is restore-from-tape onto replacement hardware at the Cremorne campus: "
    ...
    ex.para(doc, "The strategy does not meet the targets. The offsite tape-retrieval step (4–8 hours) is the "
    ...
    ex.para(doc, "Closing the RTO/RPO gap is an explicit driver of the planned LMS cloud migration. A "
    ...
    ex.para(doc, "A major LMS incident is identified through system monitoring and the ICT helpdesk. The ICT "
                 "Manager declares a disaster and authorises the recovery, notifying affected stakeholders.")
    h3("6.2 Recovery steps")
    ex.para(doc, "To completely recover the LMS from backup, the following steps are executed in sequence:")
    ex.etable(doc, ["Step", "Action", "Owner", "Duration"],
    ...
    ex.para(doc, "The plan does not currently meet the strategic objectives: the cumulative recovery time of "
    ...
    h3("7.1 Review")
    ex.para(doc, "This plan is reviewed annually, or on a material change to the LMS environment or backup "
