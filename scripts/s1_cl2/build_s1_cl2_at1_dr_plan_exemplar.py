...
def build(path):
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PE 1] — overview of the developed and evaluated plan")
    add_body_paragraph(doc, "This Disaster Recovery Plan protects the YAT public website after its expansion to serve the "
    ...
    add_body_paragraph(doc, "Recovery is driven from infrastructure-as-code plus automated and cross-region backups, "
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 1.1] identify disaster recovery plan requirements according to business needs")
    add_body_paragraph(doc, "The website was migrated to AWS and hardened for in-region high availability (Multi-AZ "
    ...
    add_body_paragraph(doc, "In scope: the website's web / application tier and its database, serving both the Australian "
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 1.2] existing organisational recovery plans · [ICTCLD501 PC 1.3] vendor disaster recovery plan and SLAs")
    add_body_paragraph(doc, "Existing arrangements (PC 1.2): the only previously documented disaster recovery plan is the "
    ...
    add_body_paragraph(doc, "Vendor provisions and SLAs (PC 1.3): AWS operates each region as multiple independent "
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 2.1–2.5] · [ICTCLD501 PE 2]")
    add_body_paragraph(doc, "Method: risks were identified in a workshop with YAT ICT and a review of the expanded "
    ...
    add_body_paragraph(doc, "Recovery objectives (PC 2.1): RTO ≤ 4 hours and RPO ≤ 1 hour, carried from the "
    ...
    add_body_paragraph(doc, "Data managed (PC 2.3): the website database (approximately 15–25 GB) holds the CMS content, "
    ...
    add_body_paragraph(doc, "Risk environment: the principal risks to a public-facing, mission-critical website now "
                 "relied on by a partner institution for enrolment are assessed below.")
    add_data_table(doc, ["#", "Risk event", "Likelihood", "Impact", "Severity", "Drives"],
    ...
    add_body_paragraph(doc, "Severity of impact and disruption (PC 2.4): RE1 and RE3 are the most severe — both can take "
    ...
    add_body_paragraph(doc, "Plan exclusions (PC 2.2): single-component and single-AZ failures are excluded — they are "
    ...
    add_body_paragraph(doc, "Outcomes (PC 2.5): this impact analysis is recorded in YAT ICT shared documentation per the "
                 "Backup and Retention and Records Management policies, and is the basis for the recovery "
                 "strategy in §5.")

    # 5 Recovery Strategy
    h1("5. Recovery Strategy")
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 3.1–3.4]")
    add_body_paragraph(doc, "Range of solutions evaluated (PC 3.1): the four standard cloud DR strategies were "
                 "assessed against the RTO/RPO targets and cost.")
    add_data_table(doc, ["Option", "RTO", "RPO", "Relative cost", "Fit"],
    ...
    add_body_paragraph(doc, "Recommended approach: Backup & Restore into Melbourne (ap-southeast-4). Automated RDS "
                 ...
                 "complexity.")
    add_body_paragraph(doc, "Vendor protections and risk prioritisation (PC 3.2): the strategy leans on AWS-native "
    ...
    add_body_paragraph(doc, "Insurance (PC 3.3): YAT holds a cyber and business-interruption insurance policy. It was "
    ...
    add_body_paragraph(doc, "Other components (PC 3.4): the strategy also includes a recovery runbook, a declaration and "
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 4.1–4.3] · [ICTCLD501 PE 3]")
    add_body_paragraph(doc, "Alignment to risk (PC 4.1): the recovery actions below are aligned to the prioritised risks "
    ...
    add_body_paragraph(doc, "Detection and declaration: a region-level event is detected by CloudWatch alarms, the AWS "
                 ...
                 "runbook.")
    add_body_paragraph(doc, "Recovery steps, timelines and service providers (PC 4.2):")
    add_data_table(doc, ["Step", "Action", "Owner", "Target (cumulative)"],
    ...
    add_body_paragraph(doc, "How the plan meets the objectives (PE 3): RPO ≤ 1 h is met because point-in-time backups and "
                 ...
                 "handed it over).")
    add_body_paragraph(doc, "Documentation (PC 4.3): this plan is the documented disaster recovery plan; it is versioned, "
    ...
    add_uoc_evidence_tag(doc, "[ICTCLD501 PC 5.1–5.4] — conducted and observed at the AT1 Part C presentation; this section records the outcome")
    add_body_paragraph(doc, "Walkthrough (PC 5.1): the plan was presented to the YAT ICT Manager and stakeholders at the "
    ...
    add_body_paragraph(doc, "Feedback sought and responded to (PC 5.2):")
    ...
    add_body_paragraph(doc, "Lodgement (PC 5.3): the approved plan is lodged in YAT ICT shared documentation under the "
    ...
    add_body_paragraph(doc, "Sign-off (PC 5.4):")
    ...
    add_body_paragraph(doc, "Written contextual responses for ICTCLD501 — the mandatory knowledge-evidence location for "
